    ...
    <div class="card-body">
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          ...
          <input id="inputTitle" type="text" name="title" placeholder="Nhập Tên"  value="{{$product->title}}" class="form-control">
          @error('title')
        ...
        <div class="form-group">
          ...
          <input id="price" type="number" name="price" placeholder="Giá Tiền"  value="{{$product->price}}" class="form-control">
          @error('price')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <input id="discount" type="number" name="discount" min="0" max="100" placeholder="% Giảm"  value="{{$product->discount}}" class="form-control">
          @error('discount')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        <div class="form-group">
          ...
          <select name="size[]" class="form-control selectpicker"  multiple data-live-search="true">
              ...
              @foreach($items as $item)              
                ...
                // dd($data);
                @endphp
              <option value="EP"  @if( in_array( "EP",$data ) ) selected @endif>E.Point</option>
              <option value="NOEP"  @if( in_array( "NOEP",$data ) ) selected @endif>Acc Mỹ</option>
        ...
        <div class="form-group">
          ...
          <select name="condition" class="form-control">
              ...
              <option value="default" {{(($product->condition=='default')? 'selected':'')}}>Thường</option>
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            ...
            <option value="inactive" {{(($product->status=='inactive')? 'selected' : '')}}>Vô Hiệu Hóa</option>
        </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
            ...
            if(cat_id !=null){
                ...
                $.ajax({
                    ...
                    success:function(response){
                        ...
                        var html_option="<option value=''>--Loại ACC--</option>";
                        if(response.status){
